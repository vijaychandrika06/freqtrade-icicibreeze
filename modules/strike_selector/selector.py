...
class StrikeSelector:
    ...
    def select_strikes(
        ...
        for row in chain.rows:
            # 1. Price sanity
            if row.ltp is None or row.ltp <= 0: continue
            
            # 2. Calc Greeks (On demand cost)
            # time_to_expiry should ideally come from expiry calc, passed in estimate for now
            
            iv = calculate_iv(row.ltp, spot, row.strike, time_to_expiry_years, risk_free_rate, chain.right)
            greeks = {}
            if iv:
                greeks = calculate_greeks(iv, spot, row.strike, time_to_expiry_years, risk_free_rate, chain.right)
    ...
    def select_best_direction_and_strikes(self, ...):
         # If needed for scanner orchestration
         pass
