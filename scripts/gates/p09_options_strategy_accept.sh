...
bash scripts/lib/ft_cmd.sh download-data -c "$CONFIG_FILE" --userdir user_data --timeframes "$TIMEFRAME" --days "$DAYS" || finish_gate $?
...
if [ "$GATE_MODE" == "pos" ]; then
    ...
    bash scripts/lib/ft_cmd.sh backtesting -c "$CONFIG_FILE" --userdir user_data --strategy IndiaOptionsAutoStrategy --timeframe "$TIMEFRAME" $RANGE_ARG || finish_gate $?
